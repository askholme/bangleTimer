// Based on bold clock https://github.com/espruino/BangleApps/blob/master/apps/boldclk/bold_clock.js
load('timer.boot.js');
var hour_hand = {
...
function showTimeLeft() {
  ...
  n = timeToNext / (settings.interval*60*1000);
  console.log(' share of interval to next: '+n);
  g.setColor(1,0,0);
  g.fillCircle(120, 120, 95);
  drawSlice(n,1);
  g.setColor(0,1,0);
  drawSlice(0,n);
}
...
Bangle.setUI({
  mode : "custom",
  btn : showTimeLeft, // optional - handler for 'button' events (n==1 on Bangle.js 2, n==1/2/3 depending on button for Bangle.js 1)
  clock : 0 // optional - if set the behavior of 'clock' mode is added (does not override btn if defined)
});

// show launcher if one swipes right, up, right,left, down
var swipeCounter = 0;
Bangle.on('swipe', function (LR,UD) {
  if (LR==1 && UD==0 && swipeCounter==0) {
    swipeCounter++;
  } else if (swipeCounter==1 && LR==0 && UD==-1) {
    swipeCounter++;
  } else if (swipeCounter==2 && LR==1 && UD==0) {
    swipeCounter++;
  } else if (swipeCounter==3 && LR==-1 && UD==0) {
    swipeCounter++;
  } else if (swipeCounter==4 && LR==0 && UD==1) {
    Bangle.showLauncher();
  } else {
    swipeCounter=0;
  }
